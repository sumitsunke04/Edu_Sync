  ...
  const handleStudentTotalInfo = async (studId) => {
    ...
    data.practicals.forEach((obj) => {
      if (obj.pracsubname !== undefined) {
        ...
        delete obj.pracsubname; // Remove the old key `pracsubname`
      }
    });
    const netdata = data.subjects.concat(data.practicals);
    console.log("Netdata : ", netdata);
    setData(netdata);
    console.log(data);
    setTypefor("studentTotalInfo");
  };

  const handleApprove = async (studentId, sub, forLab, newst) => {
    const data = await Approve(studentId, sub, forLab, newst);
    ...
    ...(type === "MySubjects"
      ? [
          ...
          columnHelper.accessor("teacher_id.fname", {
            cell: (info) => (
              <span>
                {info.row.original.teacher_id.fname}{" "}
                {info.row.original.teacher_id.lname}
              </span>
            ),
            header: "Teacher Name",
          }),
            ...
            cell: (info) => (
              <button
                ...
                onClick={() => {
                  ...
                  handleApprove(
                    info.row.original.std_id,
                    info.row.original.subname,
                    false,
                    !info.getValue()
                  );
                }}
            ...
            cell: (info) => {
              ...
              return (
                <button
                  className="bg-indigo-100 text-black rounded pl-2 pr-2 justify-center items-center"
                  onClick={() => {
                    handleStudentTotalInfo(info.getValue());
                  }}
                >
                  Show Data
                </button>
